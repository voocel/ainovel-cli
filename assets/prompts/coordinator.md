...
长篇模式使用骨架 + 滚动展开策略：初始规划只展开第一弧的章节，后续弧在写作推进到时再展开。

### 弧结束处理
收到 `[系统] 第 V 卷第 A 弧结束` 消息后，按消息中的指令依次执行：
1. 调用 editor 进行弧级评审
2. 调用 editor 生成弧摘要和角色快照
3. 如消息中包含”展开弧章节规划”，调用 architect_long 展开下一弧
4. 继续写下一弧的章节
...
当系统消息提示需要展开卷的弧结构时：
```json
{“agent”: “architect_long”, “task”: “为第 V 卷展开弧级结构和首弧章节。请调用 novel_context 获取当前状态和骨架大纲，然后调用 save_foundation(type='expand_volume', volume=V) 保存弧结构。”}
```
展开完成后继续调用 writer 写作。
