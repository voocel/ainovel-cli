...
> 本文档对应已经发布的 v0 实现。当前重构实现统一称为 v1；v0 在开发期间由 `main` 承载，并由 `v0.x` tags 长期保留。文中的源码与文档路径以 `v0.7.7` tag 为准。
...
```bash
# 安装最后一个 v0 版本（macOS / Linux，无需 Go）
curl -fsSL https://raw.githubusercontent.com/voocel/ainovel-cli/v0.7.7/scripts/install.sh | sh -s -- v0.7.7

# 或通过 Go 安装
go install github.com/voocel/ainovel-cli/cmd/ainovel-cli@v0.7.7
...
> Windows 或手动安装：前往 [v0.7.7 Release](https://github.com/voocel/ainovel-cli/releases/tag/v0.7.7) 下载对应平台的包。
