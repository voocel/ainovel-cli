...
宿主已知的事实不做成模型参数（D60）：审阅裁定只通过 `review_key` 引用已保存的问题列表，章节范围取自任务输入、发现取自记录当前版本，由宿主装配完整裁定；意图核验与阻塞问题仍执行原有严格校验。章节、审阅记录、候选等工作稿整篇写入时不带版本前提，只有按块编辑（`workspace_replace_block`）需要传入刚读到的版本。
