...
// AskUserResponse 用户回答Kết quả。
type AskUserResponse struct {
	Answers map[string]string // question text → 用户Chọn的答案
	Notes   map[string]string // question text → Tuỳ chỉnhNhập（选"Khác"时）
}

// AskUserHandler 阻塞Chờ người dùng回答，由 CLI 或 TUI 注入具体实现。
...
// 交互工具：阻塞Chờ người dùng回答，显然不能并发调度。
...
func (t *AskUserTool) Description() string {
	return "当需求信息不足、且缺失信息会明显影响规划方向时，向用户提出 1-4 个结构化问题。每个问题必须包含 header、question 和 2-4 个选项；用户可选预设项，也可自由补充。Quay lạiKết quả是可直接阅读的中文Tóm tắt，格式类似：用户回答：[篇幅] 长篇；[重心] 剧情升级（补充：不要后宫）。只有在Không thể稳定判断篇幅、主线重心、关键约束或明确偏好时才使用；不要把能自行合理推断的问题都抛给用户。"
}
...
func (t *AskUserTool) Schema() map[string]any {
	...
	question := schema.Object(
		...
		schema.Property("multiSelect", schema.Bool("Có czy không允许多选")),
	)
...
func (t *AskUserTool) Execute(ctx context.Context, args json.RawMessage) (json.RawMessage, error) {
	...
	if err := validateQuestions(a.Questions); err != nil {
		return json.Marshal(fmt.Sprintf("参数校验Thất bại: %s", err))
	}
	...
	if h == nil {
		return json.Marshal("Hiện tại环境Không hỗ trợ交互式询问，Vui lòng根据你的判断自行决策并Tiếp tục。")
	}

	resp, err := h(ctx, a.Questions)
	if err != nil {
		return json.Marshal(fmt.Sprintf("用户交互Thất bại: %s。Vui lòng根据你的判断自行决策并Tiếp tục。", err))
	}
...
func validateQuestions(questions []Question) error {
	if len(questions) == 0 {
		return fmt.Errorf("至少Cần一个问题")
	}
	if len(questions) > 4 {
		return fmt.Errorf("最多4个问题，Hiện tại %d 个", len(questions))
	}
	for i, q := range questions {
		if q.Question == "" {
			return fmt.Errorf("问题 %d: 问题文本不能为Rỗng", i+1)
		}
		if q.Header == "" {
			return fmt.Errorf("问题 %d: header 不能为Rỗng", i+1)
		}
		if utf8.RuneCountInString(q.Header) > 12 {
			return fmt.Errorf("问题 %d: header %q 超过12字符", i+1, q.Header)
		}
		if len(q.Options) < 2 || len(q.Options) > 4 {
			return fmt.Errorf("问题 %d: Cần2-4个选项，Hiện tại %d 个", i+1, len(q.Options))
		}
		for j, opt := range q.Options {
			if opt.Label == "" {
				return fmt.Errorf("问题 %d 选项 %d: label 不能为Rỗng", i+1, j+1)
			}
			if opt.Description == "" {
				return fmt.Errorf("问题 %d 选项 %d: description 不能为Rỗng", i+1, j+1)
			}
		}
	}
	return nil
...
func formatAnswers(questions []Question, resp *AskUserResponse) string {
	if resp == nil || len(resp.Answers) == 0 {
		return "用户未提供回答，Vui lòng根据你的判断自行决策并Tiếp tục。"
	}
	var parts []string
