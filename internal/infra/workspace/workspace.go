...
func (s *Service) PutChapter(
	...
	if err != nil {
		return model.WorkspaceArtifact{}, fmt.Errorf("encode workspace chapter: %w", err)
	}
	ref := model.DocumentRef{Kind: model.DocumentManuscript, ID: chapter.ID}
	if err := model.ValidateDocumentContent(ref, payload); err != nil {
		return model.WorkspaceArtifact{}, err
	}
	return s.store.PutWorkspaceArtifact(ctx, model.WorkspaceArtifact{
...
func (s *Service) ReplaceChapterBlock(
	...
	if strings.TrimSpace(blockID) == "" || strings.TrimSpace(text) == "" {
		return model.WorkspaceArtifact{}, fmt.Errorf("block id and text are required: %w", model.ErrInvalid)
	}
	...
	if artifact.Version != expectedVersion {
		return model.WorkspaceArtifact{}, fmt.Errorf(
			...
			expectedVersion, artifact.Version, model.ErrWorkspaceConflict,
		)
	}
	if artifact.MediaType != ChapterMediaType {
		return model.WorkspaceArtifact{}, fmt.Errorf("artifact %q is %q, not a chapter: %w", key, artifact.MediaType, model.ErrInvalid)
	}
	var chapter model.ManuscriptChapter
	if err := json.Unmarshal(artifact.Content, &chapter); err != nil {
		return model.WorkspaceArtifact{}, fmt.Errorf("decode workspace chapter: %w", err)
	}
	ref := model.DocumentRef{Kind: model.DocumentManuscript, ID: chapter.ID}
	if err := model.ValidateDocumentContent(ref, artifact.Content); err != nil {
		return model.WorkspaceArtifact{}, fmt.Errorf("validate workspace chapter: %w", err)
	}
	found := false
