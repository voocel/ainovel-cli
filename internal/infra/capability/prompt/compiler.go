...
func Compile(request CompileRequest) (Compiled, error) {
	...
	if request.BaseRevision < model.InitialRevision || request.ProjectOverlayRevision < model.InitialRevision {
		return Compiled{}, fmt.Errorf("compile revisions cannot be negative: %w", model.ErrInvalid)
	}
...
func CacheKey(projectID, workerProfile, executionProfileDigest, sessionLineage string) (string, error) {
	...
	for _, part := range parts {
		if strings.TrimSpace(part) == "" {
			return "", fmt.Errorf("cache key parts are required: %w", model.ErrInvalid)
		}
	}
	return model.Digest([]byte(strings.Join(parts, "\x00"))), nil
}
...
func compileTools(input []ToolSchema) ([]ToolSchema, string, error) {
	...
	return tools, model.Digest(payload), nil
}
...
func compilePacks(input []VersionedPack, slots []Slot) ([]VersionedPack, []block, []Source, error) {
	...
	for _, pack := range packs {
		if pack.Revision <= model.InitialRevision {
			return nil, nil, nil, fmt.Errorf("pack revision must be positive: %w", model.ErrInvalid)
		}
		if err := pack.Manifest.Validate(); err != nil {
			return nil, nil, nil, err
		}
		if _, ok := seen[pack.Manifest.ID]; ok {
			return nil, nil, nil, fmt.Errorf("duplicate pack %q: %w", pack.Manifest.ID, model.ErrInvalid)
		}
...
func compileCreatorProfiles(input []VersionedCreatorProfile) ([]VersionedCreatorProfile, []block, []Source, error) {
	...
	for _, profile := range profiles {
		if profile.Revision <= model.InitialRevision || profileScopeRank(profile.Profile.Scope) < 0 {
			return nil, nil, nil, fmt.Errorf("creator profile revision or scope is invalid: %w", model.ErrInvalid)
		}
		...
		if _, ok := seen[id]; ok {
			return nil, nil, nil, fmt.Errorf("duplicate creator profile %q: %w", id, model.ErrInvalid)
		}
		seen[id] = struct{}{}
		content, err := canonicalValue(struct {
			ID               string            `json:"id"`
...
func canonicalJSON(raw json.RawMessage) (json.RawMessage, error) {
	...
	if err := decoder.Decode(&struct{}{}); err != io.EOF {
		return nil, fmt.Errorf("JSON must contain exactly one value: %w", model.ErrInvalid)
	}
