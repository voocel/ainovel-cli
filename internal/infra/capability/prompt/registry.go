...
func (r *Registry) Reload(ctx context.Context, request CompileRequest, createdAt time.Time) (Compiled, error) {
	if createdAt.IsZero() {
		return Compiled{}, fmt.Errorf("execution profile creation time is required: %w", model.ErrInvalid)
	}
...
func (r *Registry) Lint(ctx context.Context, digest string) ([]Diagnostic, error) {
	...
	if err != nil || worker.ID+"@"+worker.Version != compiled.WorkerProfile {
		return nil, fmt.Errorf("worker profile %q is unavailable: %w", compiled.WorkerProfile, model.ErrInvalid)
	}
