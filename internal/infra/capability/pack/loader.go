...
func LoadDirectory(path string) (Loaded, error) {
	...
	if strings.TrimSpace(manifest.ID) == "" || strings.TrimSpace(manifest.Version) == "" || strings.TrimSpace(manifest.Name) == "" {
		return Loaded{}, fmt.Errorf("pack id, version and name are required: %w", model.ErrInvalid)
	}
...
func finalizeLoaded(loaded Loaded) (Loaded, error) {
	...
	identity.Root = ""
	identity.Digest = ""
	digest, err := model.DigestJSON(identity)
	if err != nil {
...
func readAsset(root, relative string) ([]byte, error) {
	...
	if cleaned == "." || cleaned == ".." || strings.HasPrefix(cleaned, ".."+string(filepath.Separator)) {
		return nil, fmt.Errorf("pack asset path %q escapes the pack: %w", relative, model.ErrInvalid)
	}
	...
	for _, part := range strings.Split(cleaned, string(filepath.Separator)) {
		...
		if info.Mode()&os.ModeSymlink != 0 {
			return nil, fmt.Errorf("pack asset %q uses a symbolic link: %w", relative, model.ErrInvalid)
		}
	}
	...
	if !info.Mode().IsRegular() {
		return nil, fmt.Errorf("pack asset %q is not a regular file: %w", relative, model.ErrInvalid)
	}
...
func validateFileList(lists ...[]string) error {
	...
	for _, list := range lists {
		for _, name := range list {
			...
			if _, ok := seen[key]; ok {
				return fmt.Errorf("pack asset %q is listed more than once: %w", name, model.ErrInvalid)
			}
			seen[key] = struct{}{}
		}
	}
