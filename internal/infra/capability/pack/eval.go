...
func ParseEvals(manifest model.PackManifest) (map[string]EvalSuite, error) {
	...
	for _, path := range paths {
		...
		if _, exists := seenIDs[suite.ID]; exists {
			return nil, fmt.Errorf("duplicate pack eval id %q: %w", suite.ID, model.ErrInvalid)
		}
...
func RunEvals(manifest model.PackManifest, output string) (EvalResult, error) {
...
func (suite EvalSuite) validate() error {
	if strings.TrimSpace(suite.ID) == "" || len(suite.Checks) == 0 {
		return fmt.Errorf("eval id and checks are required: %w", model.ErrInvalid)
	}
	seen := make(map[string]struct{}, len(suite.Checks))
	for index, check := range suite.Checks {
		if strings.TrimSpace(check.ID) == "" {
			return fmt.Errorf("eval check %d requires an id: %w", index, model.ErrInvalid)
		}
		if _, exists := seen[check.ID]; exists {
			return fmt.Errorf("duplicate eval check %q: %w", check.ID, model.ErrInvalid)
		}
		seen[check.ID] = struct{}{}
		switch check.Type {
		case EvalContains, EvalNotContains:
			if check.Text == "" || check.Min != 0 {
				return fmt.Errorf("eval check %q requires text and forbids min: %w", check.ID, model.ErrInvalid)
			}
		case EvalMinRunes:
			if check.Min <= 0 || check.Text != "" {
				return fmt.Errorf("eval check %q requires a positive min and forbids text: %w", check.ID, model.ErrInvalid)
			}
		default:
			return fmt.Errorf("eval check %q has unknown type %q: %w", check.ID, check.Type, model.ErrInvalid)
		}
	}
	return nil
}
