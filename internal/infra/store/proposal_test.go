...
func TestRelocateProposalUpdatesOnlyPendingRowsAndIsFenced(t *testing.T) {
	...
	if err := s.RelocateProposal(ctx, relocated, claimed.Attempt+1, start); !errors.Is(err, model.ErrStateConflict) {
	...
	missing := relocated
	missing.ID = "missing"
	if err := s.RelocateProposal(ctx, missing, 0, start); !errors.Is(err, model.ErrNotFound) {
	...
	if err := s.RelocateProposal(ctx, relocated, 0, start); !errors.Is(err, model.ErrStateConflict) {
