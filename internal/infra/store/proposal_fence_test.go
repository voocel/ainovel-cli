...
func TestPreparedProposalCannotBeWrittenBySupersededAttempt(t *testing.T) {
	...
	proposal := pendingTestProposal(testChange("candidate", op.Target, 1, model.Patch{Document: model.DocumentRef{Kind: model.DocumentIntent, ID: "root"}, Operation: model.PatchPut, Content: []byte(`{"premise":"测试"}`)}))
	proposal.OperationID = op.ID
	if _, err := s.SaveExecutionProposal(ctx, proposal, first.Attempt); !errors.Is(err, model.ErrStateConflict) {
