...
func (s *Store) SavePreferenceCandidate(
	...
	candidate model.PreferenceCandidate,
	createdAt time.Time,
) (model.PreferenceCandidateRecord, error) {
	if strings.TrimSpace(profileID) == "" || strings.TrimSpace(scope) == "" || createdAt.IsZero() {
		return model.PreferenceCandidateRecord{}, fmt.Errorf("preference profile, scope and time are required: %w", model.ErrInvalid)
	}
	...
	if err != nil {
		return model.PreferenceCandidateRecord{}, fmt.Errorf("encode preference candidate: %w", err)
	}
	digest := model.Digest(payload)
	...
	if err != nil {
		return model.PreferenceCandidateRecord{}, fmt.Errorf("save preference candidate: %w", err)
	}
	rows, err := result.RowsAffected()
	if err != nil {
		return model.PreferenceCandidateRecord{}, fmt.Errorf("inspect preference candidate save: %w", err)
	}
	...
	if rows == 0 {
		storedDigest, err := model.DigestJSON(record.Candidate)
		if err != nil {
			return model.PreferenceCandidateRecord{}, err
		}
		if storedDigest != digest {
			return model.PreferenceCandidateRecord{}, fmt.Errorf("preference candidate %q: %w", candidate.ID, model.ErrIdempotencyConflict)
		}
	}
	return record, nil
}

func (s *Store) GetPreferenceCandidate(ctx context.Context, profileID, scope, candidateID string) (model.PreferenceCandidateRecord, error) {
	var record model.PreferenceCandidateRecord
	var payload []byte
	var createdAt int64
	...
	if errors.Is(err, sql.ErrNoRows) {
		return model.PreferenceCandidateRecord{}, model.ErrNotFound
	}
	if err != nil {
		return model.PreferenceCandidateRecord{}, fmt.Errorf("read preference candidate: %w", err)
	}
	...
	if err := json.Unmarshal(payload, &record.Candidate); err != nil {
		return model.PreferenceCandidateRecord{}, fmt.Errorf("decode preference candidate: %w", err)
	}
	return record, nil
}

func (s *Store) ListPreferenceCandidates(ctx context.Context, profileID, scope string) ([]model.PreferenceCandidateRecord, error) {
	...
	records := make([]model.PreferenceCandidateRecord, 0, len(ids))
	for _, id := range ids {
...
func (s *Store) ConfirmPreferenceCandidate(ctx context.Context, profileID, scope, candidateID string, at time.Time) error {
	if at.IsZero() {
		return fmt.Errorf("preference confirmation time is required: %w", model.ErrInvalid)
	}
	result, err := s.db.ExecContext(ctx, `
