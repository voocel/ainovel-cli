...
func TestCommitChangeSetAndReadHistory(t *testing.T) {
	...
	intent := model.DocumentRef{Kind: model.DocumentIntent, ID: "root"}

	first := testChange("change-1", target, 0, model.Patch{
		Document: intent, Operation: model.PatchPut, Content: json.RawMessage(`{"premise":"凡人修仙"}`),
	})
	...
	second := testChange("change-2", target, 1, model.Patch{Document: intent, Operation: model.PatchDelete})
	...
	if _, err := s.GetDocument(ctx, target, intent, 2); !errors.Is(err, model.ErrNotFound) {
		t.Fatalf("get deleted document error = %v, want model.ErrNotFound", err)
	}
...
func TestCommitChangeSetIsIdempotent(t *testing.T) {
	...
	ctx := context.Background()
	target := model.AuthorityTarget{Kind: model.AuthorityProject, ID: "book-1"}
	change := testChange("same-id", target, 0, model.Patch{
	...
	if _, err := commitTestProposal(ctx, s, change); !errors.Is(err, model.ErrIdempotencyConflict) {
		t.Fatalf("changed input error = %v, want model.ErrIdempotencyConflict", err)
	}
}
...
func TestRevisionConflictDoesNotWriteDocuments(t *testing.T) {
	...
	ctx := context.Background()
	target := model.AuthorityTarget{Kind: model.AuthorityProject, ID: "book-1"}
	intent := testChange("intent", target, 0, model.Patch{
		Document:  model.DocumentRef{Kind: model.DocumentIntent, ID: "root"},
		Operation: model.PatchPut,
		Content:   json.RawMessage(`{"premise":"凡人修仙"}`),
	})
	...
	planRef := model.DocumentRef{Kind: model.DocumentPlan, ID: "ending"}
	stale := testChange("stale", target, 0, model.Patch{
		Document: planRef, Operation: model.PatchPut, Content: json.RawMessage(`{"ending":"飞升"}`),
	})
	if _, err := commitTestProposal(ctx, s, stale); !errors.Is(err, model.ErrRevisionConflict) {
		t.Fatalf("stale commit error = %v, want model.ErrRevisionConflict", err)
	}
	if _, err := s.GetDocument(ctx, target, planRef, 1); !errors.Is(err, model.ErrNotFound) {
		t.Fatalf("stale document error = %v, want model.ErrNotFound", err)
	}
}
...
func TestCommitRollsBackEveryWriteOnPatchFailure(t *testing.T) {
	...
	change := testChange("atomic", target, 0,
		...
		model.Patch{
			Document:  model.DocumentRef{Kind: model.DocumentPlan, ID: "fail"},
			Operation: model.PatchPut,
...
func TestConcurrentCommitsAllowOneRevisionWinner(t *testing.T) {
	...
	changes := []model.Proposal{
		testChange("left", target, 0, model.Patch{
			Document:  model.DocumentRef{Kind: model.DocumentPlan, ID: "left"},
			Operation: model.PatchPut,
			Content:   json.RawMessage(`{"path":"left"}`),
		}),
		testChange("right", target, 0, model.Patch{
			Document:  model.DocumentRef{Kind: model.DocumentPlan, ID: "right"},
			Operation: model.PatchPut,
...
func TestCommittedRevisionSurvivesReopen(t *testing.T) {
	...
	target := model.AuthorityTarget{Kind: model.AuthorityProject, ID: "book-1"}
	chapter := model.DocumentRef{Kind: model.DocumentManuscript, ID: "chapter-1"}

	s, err := Open(ctx, path)
	if err != nil {
		t.Fatalf("open first store: %v", err)
	}
	change := testChange("chapter", target, 0, model.Patch{
		Document: chapter, Operation: model.PatchPut, Content: json.RawMessage(`{"title":"第一章"}`),
	})
	if _, err := commitTestProposal(ctx, s, change); err != nil {
...
func TestListDocumentsReturnsLatestLiveVersions(t *testing.T) {
	...
	first := testChange("first", target, 0,
		model.Patch{Document: model.DocumentRef{Kind: model.DocumentPlan, ID: "arc-1"}, Operation: model.PatchPut, Content: json.RawMessage(`{"title":"旧标题"}`)},
		model.Patch{Document: model.DocumentRef{Kind: model.DocumentPlan, ID: "arc-2"}, Operation: model.PatchPut, Content: json.RawMessage(`{"title":"第二弧"}`)},
	)
	if _, err := commitTestProposal(ctx, s, first); err != nil {
		t.Fatalf("commit initial plan: %v", err)
	}
	second := testChange("second", target, 1,
		model.Patch{Document: model.DocumentRef{Kind: model.DocumentPlan, ID: "arc-1"}, Operation: model.PatchPut, Content: json.RawMessage(`{"title":"新标题"}`)},
		model.Patch{Document: model.DocumentRef{Kind: model.DocumentPlan, ID: "arc-2"}, Operation: model.PatchDelete},
	)
	if _, err := commitTestProposal(ctx, s, second); err != nil {
		t.Fatalf("commit revised plan: %v", err)
	}

	documents, err := s.ListDocuments(ctx, target, model.DocumentPlan, 2)
	if err != nil {
...
func testChange(id string, target model.AuthorityTarget, base model.Revision, patches ...model.Patch) model.Proposal {
	decidedAt := time.Date(2026, 8, 18, 0, 0, 1, 0, time.UTC)
	return model.Proposal{
		ID:            id,
		Target:        target,
		BaseRevision:  base,
		Author:        model.Author{Kind: model.AuthorUser, ID: "user-1"},
		Reason:        "test change",
		Patches:       patches,
		ApprovalState: model.ApprovalApproved,
		DecidedBy:     &model.Author{Kind: model.AuthorUser, ID: "user-1"},
		DecidedAt:     &decidedAt,
		CreatedAt:     time.Date(2026, 8, 18, 0, 0, 0, 0, time.UTC),
	}
}

func commitTestProposal(ctx context.Context, s *Store, proposal model.Proposal) (model.ChangeSet, error) {
	if _, err := s.GetProposal(ctx, proposal.ID); errors.Is(err, model.ErrNotFound) {
		pending := proposal
