...
func publishSafetyArtifact(t *testing.T, s *Store, op, content string) domain.Artifact {
	t.Helper()
	...
	return domain.Artifact{ID: domain.ArtifactID(op, "asset"), ProjectID: "book-1", Digest: digest, Size: size, MediaType: "text/plain", OperationID: op, Attempt: 1, CreatedAt: time.Now()}
}

func seedSafetyRun(t *testing.T, s *Store) {
	t.Helper()
	now := operationTime()
	_, err := s.CreateCreationRun(context.Background(), domain.CreationRun{ID: "run:book-1", ProjectID: "book-1", Goal: domain.NovelGoal{Premise: "测试", TargetChapters: 1}.Goal(), Strategy: domain.CreationRunStrategy{PlanWindowChapters: 1, ReviewCadence: domain.ReviewPerPlanWindow, AutoRepairBudget: 1}, Preset: domain.CreationRunPreset{Source: "test", Digest: "test", Approval: domain.ApprovalAuto}, State: domain.RunRunning, CreatedAt: now, UpdatedAt: now})
	if err != nil {
		t.Fatal(err)
	}
}
...
func TestArtifactDatabasesHaveIndependentOwnership(t *testing.T) {
	...
	if err = a.SaveExecutionArtifacts(ctx, []domain.Artifact{artifact}, "op", 1); err != nil {
		t.Fatal(err)
	}
...
func TestArtifactPublicationPinSurvivesCollectionFromAnotherStore(t *testing.T) {
	...
	if err = a.SaveExecutionArtifacts(ctx, []domain.Artifact{changed}, "op", 1); !errors.Is(err, ErrIdempotencyConflict) {
...
func TestArtifactDigestBoundaryRejectsMalformedValues(t *testing.T) {
	...
	for _, digest := range []string{"x", "../secret", string(make([]byte, 64))} {
		a := domain.Artifact{ID: "op/a", ProjectID: "book-1", Digest: digest, MediaType: "text/plain", OperationID: "op", Attempt: 1, CreatedAt: now}
		if err := s.SaveExecutionArtifacts(ctx, []domain.Artifact{a}, "op", 1); !errors.Is(err, domain.ErrInvalid) {
			t.Fatalf("invalid digest accepted: %v", err)
		}
		if _, err := s.ReadArtifact(digest); !errors.Is(err, domain.ErrInvalid) {
