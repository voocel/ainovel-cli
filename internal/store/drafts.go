...
// SaveDraft Lưu整章Bản nháp到 drafts/{ch}.draft.md。
...
// AppendDraft 追加内容到现有Bản nháp（续写模式）。
...
// LoadChapterContent 加载ChươngBản nhápChính văn及字数。
...
// SaveFinalChapter Lưu最终ChươngChính văn到 chapters/{ch}.md。
...
// LoadChapterRange Đọc指定范围的终稿原文片段。
...
// ExtractDialogue 从已NộpChương中提取指定角色的对话片段。
...
// ExtractStyleAnchors 从已NộpChương中提取代表性段落作为风格锚点。
// maxCompletedChapter 由调用方传入，避免跨域依赖。
