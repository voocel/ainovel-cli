...
func TestArtifactPublishIsAtomicAndIdempotent(t *testing.T) {
	...
	if _, _, err := aborted.Publish(); !errors.Is(err, domain.ErrInvalid) {
		t.Fatalf("publish after abort err = %v", err)
	}
	if _, err := s.ReadArtifact("00" + digest[2:]); !errors.Is(err, ErrNotFound) {
...
func TestSaveExecutionArtifactsRequiresPublishedObjectAndActiveAttempt(t *testing.T) {
	...
	artifact := domain.Artifact{
		ID: domain.ArtifactID("op", "cover"), ProjectID: "book-1", Digest: digest, MediaType: "image/png",
		Size: size, OperationID: "op", Attempt: 1, CreatedAt: now,
	}
	unpublished := artifact
	unpublished.Digest = "00" + digest[2:]
	if err := s.SaveExecutionArtifacts(ctx, []domain.Artifact{unpublished}, "op", 1); !errors.Is(err, ErrNotFound) {
		t.Fatalf("unpublished object err = %v", err)
	}
	wrongSize := artifact
	wrongSize.Size = size + 1
	if err := s.SaveExecutionArtifacts(ctx, []domain.Artifact{wrongSize}, "op", 1); !errors.Is(err, domain.ErrInvalid) {
		t.Fatalf("size mismatch err = %v", err)
	}
	foreign := artifact
	foreign.ProjectID = "other-book"
	if err := s.SaveExecutionArtifacts(ctx, []domain.Artifact{foreign}, "op", 1); !errors.Is(err, domain.ErrInvalid) {
		t.Fatalf("foreign project err = %v", err)
	}
	if err := s.SaveExecutionArtifacts(ctx, []domain.Artifact{artifact}, "op", 1); err != nil {
		t.Fatalf("save: %v", err)
	}
	...
	if _, err := s.GetArtifact(ctx, "op/missing"); !errors.Is(err, ErrNotFound) {
...
func TestArtifactGarbageCollectionSparesReferencedStagingAndYoungObjects(t *testing.T) {
	...
	if err := s.SaveExecutionArtifacts(ctx, []domain.Artifact{{
		ID: domain.ArtifactID("op", "kept"), ProjectID: "book-1", Digest: referenced, MediaType: "text/plain",
...
func TestCancelBeforeArtifactMetadataCommitRejectsSave(t *testing.T) {
	...
	if _, err := s.TransitionOperation(ctx, "op", domain.OperationRunning, domain.OperationCancelled, "cancelled by user", now.Add(time.Minute)); err != nil {
	...
	if err := s.SaveExecutionArtifacts(ctx, []domain.Artifact{artifact}, "op", claimed.Attempt); !errors.Is(err, ErrStateConflict) {
		t.Fatalf("save after cancel err = %v, want ErrStateConflict", err)
	}
	if _, err := s.GetArtifact(ctx, artifact.ID); !errors.Is(err, ErrNotFound) {
