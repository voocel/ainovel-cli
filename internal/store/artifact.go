...
func (s *Store) NewArtifactWriter(operationID string, attempt int) (*ArtifactWriter, error) {
	if strings.TrimSpace(operationID) == "" || attempt <= 0 {
		return nil, fmt.Errorf("artifact writer requires operation and positive attempt: %w", domain.ErrInvalid)
	}
...
func (w *ArtifactWriter) Write(p []byte) (int, error) {
	if w.file == nil {
		return 0, fmt.Errorf("artifact writer is closed: %w", domain.ErrInvalid)
	}
...
func artifactStagingDir(root, operationID string) string {
	return filepath.Join(root, "staging", domain.Digest([]byte(operationID)))
}
...
func (s *Store) ReadArtifact(digest string) ([]byte, error) {
	if err := domain.ValidateArtifactDigest(digest); err != nil {
		return nil, err
	}
...
func (s *Store) SaveExecutionArtifacts(ctx context.Context, artifacts []domain.Artifact, operationID string, attempt int) error {
	...
	for _, artifact := range artifacts {
		...
		if artifact.OperationID != operationID || artifact.Attempt != attempt || artifact.ProjectID != projectID {
			return fmt.Errorf("artifact %q does not belong to operation %q attempt %d: %w", artifact.ID, operationID, attempt, domain.ErrInvalid)
		}
		if _, ok := seen[artifact.ID]; ok {
			return fmt.Errorf("duplicate artifact %q: %w", artifact.ID, domain.ErrInvalid)
		}
		seen[artifact.ID] = struct{}{}
		info, err := os.Stat(objectPath(s.artifactRoot, artifact.Digest))
		if errors.Is(err, fs.ErrNotExist) {
			return fmt.Errorf("artifact %q object %s is not published: %w", artifact.ID, artifact.Digest, ErrNotFound)
		}
		if err != nil {
			return fmt.Errorf("stat artifact object: %w", err)
		}
		if info.Size() != artifact.Size {
			return fmt.Errorf("artifact %q size %d does not match object %d: %w", artifact.ID, artifact.Size, info.Size(), domain.ErrInvalid)
		}
		...
		if rows != 1 {
			return fmt.Errorf("artifact %q already identifies different content or evidence: %w", artifact.ID, ErrIdempotencyConflict)
		}
...
func (s *Store) GetArtifact(ctx context.Context, id string) (domain.Artifact, error) {
	...
	if err != nil {
		return domain.Artifact{}, fmt.Errorf("read artifact: %w", err)
	}
	return artifact, nil
}
...
func scanArtifact(scanner rowScanner) (domain.Artifact, error) {
	...
	if err := json.Unmarshal(basis, &artifact.Basis); err != nil {
		return domain.Artifact{}, fmt.Errorf("decode artifact basis: %w", err)
	}
...
func (s *Store) CollectArtifactGarbage(ctx context.Context, now time.Time, grace time.Duration) (int, error) {
	if now.IsZero() || grace < 0 {
		return 0, fmt.Errorf("garbage collection requires time and non-negative grace: %w", domain.ErrInvalid)
	}
	...
	if _, err := tx.ExecContext(ctx, `DELETE FROM artifact_publications
		...
		)`, cutoff.UnixMilli(), domain.OperationRunning); err != nil {
	...
	rows, err := tx.QueryContext(ctx, `SELECT id, attempt FROM operations WHERE state = ?`, domain.OperationRunning)
	if err != nil {
		return removed, err
	}
	for rows.Next() {
		...
		active[domain.Digest([]byte(id))] = attempt
	}
	if err := rows.Err(); err != nil {
		rows.Close()
		return removed, err
