...
type CastEntry struct {
	...
	// Aliases Hiện tại没有写入通道；预留给将来的"用户 steer 合并别名"工具
	...
	// 保留显式字段方便 UI/JSON 直接读，Không có需每次重算。
	AppearanceCount    int   `json:"appearance_count"`
	AppearanceChapters []int `json:"appearance_chapters"`
	// Promoted 标记此条目已升格到 characters.json。RecentActive 会Bỏ qua这些条目，
	// 避免与核心档案重复召回。Hiện tại升格通道未实现，字段为预留 hook。
	Promoted bool `json:"promoted,omitempty"`
}

// CastIntro 是 Writer 在 commit_chapter 时对Mới出场角色的简介声明。
// 仅在该名字首次出现或 ledger 中 BriefRole 仍为Rỗng时才被采用。
