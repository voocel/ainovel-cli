...
func TestReplaceChapterBlockUsesStableIDAndWorkspaceVersion(t *testing.T) {
	...
	if _, err := authorityStore.ClaimNextOperation(ctx, "worker-1", time.Minute, now.Add(time.Second)); err != nil {
	...
	artifact, err := workspace.PutChapter(ctx, operation.ID, "chapter/chapter-1", chapter, 0, now.Add(2*time.Second))
	if err != nil {
		t.Fatalf("put chapter: %v", err)
	}
	artifact, err = workspace.ReplaceChapterBlock(
		ctx, operation.ID, artifact.Key, "p-1", "新段落", artifact.Version, now.Add(3*time.Second),
	)
	if err != nil {
	...
	if _, err := workspace.ReplaceChapterBlock(
		ctx, operation.ID, artifact.Key, "p-1", "过期覆盖", 1, now.Add(4*time.Second),
	); !errors.Is(err, store.ErrWorkspaceConflict) {
		t.Fatalf("stale edit error = %v, want ErrWorkspaceConflict", err)
	}
	if _, err := workspace.ReplaceChapterBlock(
		ctx, operation.ID, artifact.Key, "missing", "内容", 2, now.Add(5*time.Second),
