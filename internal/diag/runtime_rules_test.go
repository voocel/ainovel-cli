...
// 且运行时 Finding Tất cả AutoNone（观察者纪律：只诊断不产 Action）。
func TestRuntimeFindings_Classify(t *testing.T) {
	rc := RuntimeCapture{
		Repeats: []RepeatStat{
			{Sig: "coordinator · err: InputValidationError", Count: 14}, // Lỗi循环 critical
			{Sig: "coordinator · subagent", Count: 45},                  // 正常高频工具 → 不产 Finding
			{Sig: "writer · save_plan (args invalid)", Count: 4},        // 参数Không có效 warning
		},
...
// TestRuntimeFindings_Quiet 证明Không có异常信号时不产任何运行时 Finding（零误报）。
