...
// UsageTracker 累计整个会话所有 agent 的 LLM Nhập/输出 token 与美元成本。
//
// 工作机制：
//   - 每次 agent 的 OnMessage 回调触发时调用 Record(agentName, msg)
//   - agentName 映射到 role（architect_* 归一为 architect），查 ModelSet Hiện tại该 role 绑定的Mô hình
//   - 用 models.DefaultRegistry 查Mô hình价格，按非缓存Nhập/输出/缓存读/缓存写四项累乘
//   - 注册表Không có此Mô hình时，退回 msg.Usage.Cost.Total（provider 自带，可能为 0）
//   - Mô hình热切换（/model）后续消息自动按MớiMô hình算价，Cũ消息保留Cũ成本
//
// 同时维护 per-role 维度（writer/editor/architect/coordinator）：
//   - 累计命中数据 → 整体优化效果
//   - 滑动窗最近 N 次 → 区分前期拖累 vs 稳态低命中
//   - CacheCapable 标记 → 区分"未Bật"和"真的 0% 命中"
//
// 线程安全。
type UsageTracker struct {
	...
	perModel map[string]*agentTotals // key 为 provider/model；provider Không rõ时退化为 model
	modelSet *bootstrap.ModelSet
	store    *storepkg.Store // 可为 nil（测试Cảnh），nil 时所有持久化方法静默 noop
	...
	// 始终为 nil，所有累计字段Tất cả停在 0。计数器让 UI 能直接告诉用户"是上游不返
	...
	// buffered=1：连续多次 Record Thu gọn为一次落盘信号；满了直接丢，下个 tick 一并写。
	saveCh chan struct{}

	// onCost 在每次记账后于锁外携带最Mới累计成本调用（BudgetSentinel 越线检测）。
	// 必须在并发 Record Bắt đầu前通过 SetOnCost Thiết lập，之后只读。
	onCost func(total float64)

	// onMissingUsage 在首次发现"assistant 消息Không có Usage"时调用一次（与 slog warn
	// 同时机）。预算Bật时这意味着计费盲区——成本恒 0、预算永不触发，必须喊人。
	onMissingUsage func()
}
...
//   - Saved 是按Hiện tại命中数据反算的"如果按非缓存价计费"的差额
//   - CacheCapable 仅在该 role 至少经过一次"已知支持 cache 的Mô hình"调用后置 true
...
// Record 把一条 agent 消息分发到累加 / 诊断两条Đường dẫn。
//
// 累加只看 Usage Có czy không存在——"哪条消息带 Usage" 是 agentcore/litellm adapter
// 装配细节（上游协议把 usage 放在响应顶层），未来装配规则变了也不用动这里。
// 诊断要求 Role=Assistant 且 Content 非Rỗng，避免 AbortMsg / 异常Phục hồi / tool /
...
func (t *UsageTracker) flagMissingUsage(agentName string) {
	...
	t.mu.Unlock()
	if shouldLog {
		slog.Warn("LLM 响应未携带 usage 数据，缓存/成本面板将Không có累计——通常是上游 streaming 未按 OpenAI include_usage 协议发 final usage chunk",
...
// 必须在 Host 构造期、并发 Record Bắt đầu前调用一次。
...
// 信号通道 buffered=1：连续多次 Record Thu gọn成一次LưuVui lòng求即可。
...
// provider/model 为Rỗng表示"用Hiện tại ModelSet 拿 role 对应Mô hình"（实时Đường dẫn）；非Rỗng表示
// "强制按指定Mô hình算价"（replay Đường dẫn用 session jsonl 里的 _meta）。
...
// SetOnCost 注册记账回调（携带最Mới累计成本，锁外调用）。
// 必须在 Host 构造期、并发 Record Bắt đầu前调用一次。
...
// 因为自建 backend Mô hình（mimo-v2.5-pro / 国内Proxy等）通常不在 BerriAI/litellm
// pricing 索引里，但实际 Usage 里完全有 cache 数据，UI 不该误判为"未Bật"。
...
// recentSums Quay lại滑动窗内 cacheRead 和 input 的总和，作为"近 N 次命中率"的分子分母。
...
// SavedUSD Quay lại因缓存命中节省的累计美元数。
...
// OverallRecent Quay lại滑动窗内（≤ recentSampleCap 次）的 cacheRead 总和、input 总和、样本数。
...
// OverallCacheCapable 整体Có czy không至少经过一次已知支持 cache 的Mô hình。
...
// MissingAssistantUsage Quay lại累计"收到 assistant 消息但 Usage 为 nil"的次数。
...
// Snapshot 拷贝Hiện tại累计Trạng thái为可序列化的 domain.UsageState。
// 滑动窗 samples 不进 snapshot——它是短期诊断Cửa sổ，跨进程意义不大。
...
// LoadFromStore 从 store.Usage Đọc持久化的Chụp并回填到内存。Quay lại true 表示
// Thành công加载到了一份非Rỗng（schema 匹配）的Trạng thái；false 表示Không cóTập tin或Không可用，调用方
// 应Tiếp tục走 session replay 一次性回填。
...
// SaveNow 立刻把Hiện tại snapshot 落盘。autoSaveLoop / Close Đường dẫn都通过它写。
...
// 把最后一次未Lưu的Trạng thái flush 出去。Close 通过 cancel ctx 触发 flush + Thoát。
...
// 即便用户手动 ctrl+C Thoát来不及触发 timer，ctx HủyĐường dẫn也会 flush 最后一次。
...
func (t *UsageTracker) autoSaveLoop(ctx context.Context) {
	...
	flush := func() {
		if err := t.SaveNow(); err != nil {
			slog.Warn("usage 落盘Thất bại", "module", "usage", "err", err)
		}
		pending = false
	}
	for {
...
// applyState 把持久化Chụp写回内存。仅在启动时调用（LoadFromStore / replay 后），
...
// totalsFromState 把持久化形态Hoàn nguyên为内存 agentTotals。samples 留Rỗng，重启后
// 重Mới从 0 Bắt đầu积累，几轮 Record 后即可Phục hồi"近 N 次命中率"语义。
...
// AgentUsage 是一个 agent 的累计用量Chụp（向 UI 暴露）。
...
// PerAgent Quay lại各 role 累计用量。Kết quả按 CacheRead 数量降序，未消费过 token 的 role Bỏ qua。
...
// PerModel Quay lại各Mô hình累计用量。Kết quả按成本降序，其次按Nhập量降序。
...
// resolveCost 同时Quay lại本次消息的 cost / saved / capable。
//   - cost: 注册表命中按 4 项累乘；未命中回落 provider 自带 cost
//   - saved: 仅注册表命中、CacheRead > 0、且 InputCost > CacheReadCost 时 > 0
//   - capable: 注册表命中且该Mô hình CacheReadCostPer1M > 0 → 已知支持 prompt caching
//
...
// architect_short/mid/long 都归到 architect；Khác原样Quay lại。
...
//
//	u.Input  = Tất cảNhập token，**包含** CacheRead；不含 CacheWrite
//	u.Output = 输出 token
//
...
// computeSaved 估算 CacheRead 命中相对于"按普通Nhập价计费"省下的美元。
