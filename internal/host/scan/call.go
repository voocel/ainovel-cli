...
// errTruncated 表示模型因长度停止（容量错误），携带原始文本供调用方决定失败或降批重试。
...
// errSemantic 表示无法通过重问修复的输出层失败，携带原始响应供 runner 落 failures/。
...
type callProfile struct {
	...
	// notify 可选：把请求退避重试/校验重问回显给界面；nil 时静默。
	// retryAt 非零 = 下次重试的截止时刻，UI 据此渲染逐秒倒计时。
	notify func(msg string, retryAt time.Time)
	// progress 可选：回显长时阶段的内部推进（第 N/M 份数据源）；nil 时静默。
	progress func(current, total int, msg string)
	// log 可选：扫榜专属日志（logs/scan.log）；nil 回退默认 logger。
	log *slog.Logger
}
...
// say 回显一条长时调用状态。重试退避可静默累计数分钟，不回显用户会误以为卡死。
...
// 网关的 message 常常只有一句空泛的 "Provider returned error"，这些事实才能区分
// 配置错、上游故障与限流。
...
// thinking 仅在非 Auto 时发送——对不支持 thinking 的模型发任何等级（含 off）都是非法参数。
...
// callStructured 为扫榜层适配统一结构化执行器，并把通用失败映射为扫榜工件语义。
func callStructured[T any](ctx context.Context, m callModel, contract llmcontract.Contract, systemPrompt, payload string, maxTokens int, prof callProfile, validate func(*T) error) (T, error) {
	out, err := llmcontract.Execute(ctx, m, llmcontract.Request[T]{
		...
		Hooks: llmcontract.Hooks{
			Resolved: func(res llmcontract.Resolution) {
				prof.logger().Debug("scan 结构化协议选择",
			...
			RequestRetry: func(ev llmretry.Event) {
				...
				prof.logger().Warn("scan 模型请求重试", "attempt", ev.Attempt, "delay", ev.Delay, "err", ev.Err)
			},
			Correction: func(ev llmcontract.Correction) {
				prof.say("输出校验未通过（%s），带错误反馈进行第 %d 次重问", briefErr(ev.Err), ev.Attempt+1)
				prof.logger().Warn("scan 结构化输出自愈", "attempt", ev.Attempt,
	...
	case llmcontract.FailureSafety, llmcontract.FailureContract, llmcontract.FailureProtocol:
		if failure.Raw != "" {
			return out, &errSemantic{Raw: failure.Raw, Err: fmt.Errorf("scan: %w", failure)}
		}
	case llmcontract.FailureRequest:
		if detail := modelErrDetail(failure); detail != "" {
			return out, fmt.Errorf("scan: 模型调用失败（%s）：%w", detail, failure)
		}
	}
	return out, fmt.Errorf("scan: %w", failure)
}
