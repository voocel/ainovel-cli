...
func Run(ctx context.Context, deps Deps, opts Options) (<-chan Event, error) {
	...
	go func() {
		...
		if err != nil {
			emit(StageError, 0, 0, "扫描 simulate Thư mụcThất bại", err)
			return
		}
		if len(sources) == 0 {
			emit(StageError, 0, 0, "simulate Thư mục中没有可分析的 .txt/.md/.markdown Tập tin", fmt.Errorf("no simulation sources"))
			return
		}

		existing, err := deps.Store.Simulation.Load()
		if err != nil {
			emit(StageError, 0, len(sources), "Đọc既有画像Thất bại", err)
			return
		}
		pending := pendingSources(existing, sources)
		if len(pending) == 0 {
			emit(StageDone, 0, len(sources), "画像已是最Mới，未发现Mới增或变更文章", nil)
			return
		}
		...
		for i, source := range pending {
			if err := ctx.Err(); err != nil {
				emit(StageError, i, len(pending), "用户Hủy画像分析", err)
				return
			}
			...
			if err != nil {
				emit(StageError, i+1, len(pending), "语料分析Thất bại", err)
				return
			}
		...
		if err != nil {
			emit(StageError, len(pending), len(pending), "画像合并Thất bại", err)
			return
		}
		...
		if err := deps.Store.Simulation.Save(profile); err != nil {
			emit(StageError, len(pending), len(pending), "Lưu仿写画像Thất bại", err)
			return
		}
		emit(StageDone, len(pending), len(pending), fmt.Sprintf("仿写画像已更Mới：Mới增/变更 %d 篇，累计 %d 篇", len(pending), len(profile.Corpus.Sources)), nil)
	}()
	return events, nil
}
