...
import (
	...
	bench "github.com/voocel/ainovel-cli/internal/app/workbench"
)
...
func TestWorkbenchStaleCandidateFollowsRelocationRule(t *testing.T) {
	...
	prepare := func(projectID string) (*testApp, string) {
		t.Helper()
	...
	t.Run("related directive expires the candidate", func(t *testing.T) {
		...
		snapshot, err := api.Workbench.WorkbenchSnapshot(ctx, "drift-book")
		if err != nil {
		...
		for _, entry := range snapshot.Outline {
			if entry.State == bench.ChapterPending {
				t.Fatalf("stale candidate must not mark outline pending: %#v", entry)
			}
		}
		if _, err := api.Decisions.Approve(ctx, proposalID, "user-1", testTime().Add(3*time.Hour)); !errors.Is(err, model.ErrRevisionConflict) {
	...
	t.Run("ownership change relocates the candidate", func(t *testing.T) {
		...
		if _, err := api.Projects.SetOwnership(ctx, projectdoc.SetOwnershipCommand{
			...
			Control: model.ControlGuided, Guidance: []string{"保持启程弧基调"},
			Reason: "写作中途锁定", CreatedAt: testTime().Add(2 * time.Hour),
		}); err != nil {
			t.Fatalf("drift via ownership change: %v", err)
		}
		snapshot, err := api.Workbench.WorkbenchSnapshot(ctx, "relocate-book")
		if err != nil {
			t.Fatalf("snapshot after drift: %v", err)
		}
		...
		committed, err := api.Decisions.Approve(ctx, proposalID, "user-1", testTime().Add(3*time.Hour))
...
func TestWorkbenchSnapshotAcrossLifecycle(t *testing.T) {
	...
	if _, err := api.Projects.CreateProject(ctx, projectdoc.CreateProjectCommand{
		...
		Reason: "工作台快照测试", Draft: testProjectDraft(), CreatedAt: testTime(),
	}); err != nil {
		t.Fatalf("create project: %v", err)
	}
	snapshot, err := api.Workbench.WorkbenchSnapshot(ctx, "fresh-book")
	if err != nil {
	...
	if result.RunState != model.RunWaitingUser {
	...
	for _, entry := range snapshot.Outline {
		if entry.Node.Kind != model.PlanChapter {
			continue
		}
		chapterNodes++
		if entry.State == bench.ChapterPending {
	...
	for _, suffix := range [][]string{{"chapter", "chapter-plan-1"}, {"chapter", "chapter-plan-2"}} {
		...
		if _, err := api.Decisions.Approve(ctx, runQuickID(result.RunID, suffix...)+"-proposal", "user-1", command.CreatedAt); err != nil {
	...
	if result.RunState != model.RunCompleted {
	...
	confirmedCount := 0
	for _, entry := range snapshot.Outline {
		if entry.State == bench.ChapterConfirmed {
			confirmedCount++
		}
	}
