...
func TestAddAndRetireDirectiveRoundTrip(t *testing.T) {
	...
	added, err := api.Projects.AddDirective(ctx, projectdoc.AddDirectiveCommand{
		ProjectID: "directed-book", ChangeID: "directive-1", UserID: "user-1", Reason: "提要求",
		DirectiveID: "d-1", Scope: "from_chapter:1", Text: "每章结尾留钩子",
		Constraints: &model.DirectiveConstraints{TargetWords: 3000}, CreatedAt: now.Add(time.Minute),
	})
	if err != nil || added.Revision != 2 || added.Directive.Status != model.DirectiveActive {
		t.Fatalf("add = %#v, %v", added, err)
	}

	// 同 ID 同内容重复提交幂等：不产生新 Revision。
	repeated, err := api.Projects.AddDirective(ctx, projectdoc.AddDirectiveCommand{
		ProjectID: "directed-book", ChangeID: "directive-1-again", UserID: "user-1", Reason: "重复",
		DirectiveID: "d-1", Scope: "from_chapter:1", Text: "每章结尾留钩子",
		Constraints: &model.DirectiveConstraints{TargetWords: 3000}, CreatedAt: now.Add(2 * time.Minute),
	})
	if err != nil || repeated.Revision != 2 {
		t.Fatalf("repeated add = %#v, %v", repeated, err)
	}

	bench, err := api.Workbench.WorkbenchSnapshot(ctx, "directed-book")
	if err != nil || len(bench.Directives) != 1 || bench.Directives[0].ID != "d-1" {
		t.Fatalf("workbench directives = %#v, %v", bench.Directives, err)
	}

	retired, err := api.Projects.RetireDirective(ctx, projectdoc.RetireDirectiveCommand{
		ProjectID: "directed-book", ChangeID: "retire-1", UserID: "user-1", Reason: "不再需要",
		DirectiveID: "d-1", CreatedAt: now.Add(3 * time.Minute),
	})
	if err != nil || retired.Revision != 3 || retired.Directive.Status != model.DirectiveRetired {
		t.Fatalf("retire = %#v, %v", retired, err)
	}
	again, err := api.Projects.RetireDirective(ctx, projectdoc.RetireDirectiveCommand{
	...
	project, err := api.Projects.Project(ctx, "directed-book", 0)
	if err != nil || len(project.Directives) != 1 || project.Directives[0].Status != model.DirectiveRetired {
	...
	projection, err := api.Projects.ExportProject(ctx, "directed-book", 0)
	if err != nil || len(projection.Directives) != 1 {
		t.Fatalf("export directives = %#v, %v", projection.Directives, err)
	}
	projection.Directives = append(projection.Directives, model.Directive{
		ID: "d-2", Scope: "plan_node:chapter-plan-1", Text: "第一章主角要落败", Status: model.DirectiveActive,
	})
	proposal, err := api.Projects.ImportProject(ctx, "import-directive", "user-1", "补要求", projection, now.Add(5*time.Minute))
	if err != nil {
		t.Fatalf("import: %v", err)
	}
	if _, err := api.Decisions.Approve(ctx, proposal.ID, "user-1", now.Add(6*time.Minute)); err != nil {
